...
//    box = Rect(145, 150, 30, 30);//recr(x,y,w,h)
    ...
    {
        Mat skinmask = cvCreateMat(frame.rows, frame.cols, CV_8UC1);
        skintracker(frame, box, skinmask);
        if(first_flag){
            //Mat first_frame;
...
//            printf("aver=%lf averAver=%lf\n",aver,averAver);
            //boxRatio=aver/averAver;
            //allRatio=averAll/averAverAll;
          //  printf("aver/averAver=%lf----while averAll/averAverAll=%lf\n",boxRatio,allRatio);
            //printf("aver=%lf----while averAver=%lf\n",aver,averAver);
            if(aver>0.5 && aver>averAver){
                if(foundCountFlag){
                    foundCount++;
                }
                foundCountFlag=1;
